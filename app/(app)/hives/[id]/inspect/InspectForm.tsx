...
export default function InspectForm({ farmId, hiveId, hiveCode }: Props) {
  ...
  const [colonyStrength, setColonyStrength] = useState("");
  ...
  const [diseaseSigns, setDiseaseSigns] = useState(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        `/api/farms/${farmId}/hives/${hiveId}/inspections`,
        {
          method: "POST",
          ...
          body: JSON.stringify({
            inspectionDate,
            queenPresent,
            queenStatus:       queenStatus || undefined,
            colonyStrength:    colonyStrength || undefined,
            framesWithBees:    framesWithBees !== "" ? Number(framesWithBees) : undefined,
            framesWithBrood:   framesWithBrood !== "" ? Number(framesWithBrood) : undefined,
            supersCount:       supersCount !== "" ? Number(supersCount) : undefined,
            honeyStores:       honeyStores || undefined,
            temperament:       temperament || undefined,
            diseaseSigns,
            diseaseNotes:      diseaseSigns ? diseaseNotes.trim() || undefined : undefined,
            treatmentApplied:  treatmentApplied.trim() || undefined,
    ...
    <div className="flex min-h-dvh flex-col bg-gray-50">
      ...
      <main className="flex-1 px-4 pb-8 pt-5">
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          >
            {loading ? "Duke ruajtur..." : "Ruaj inspektimin"}
          </button>
        </form>
      </main>
    </div>
